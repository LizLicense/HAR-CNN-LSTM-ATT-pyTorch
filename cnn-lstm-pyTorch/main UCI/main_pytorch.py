...
def train(model, optimizer, train_loader, test_loader):
    ...
    for e in range(args.nepoch):
        ...
        np.savetxt('result_cnn-lstm_UCI.csv', result_np, fmt='%.2f', delimiter=',')
...
def plot():
    data = np.loadtxt('result_cnn-lstm_UCI.csv', delimiter=',')
    plt.figure()
    ...
    plt.savefig('plot_cnn-lstm_UCI.png')
...
if __name__ == '__main__':
    ...
    np.savetxt('result_cnn-lstm_UCI.csv', result, fmt='%.2f', delimiter=',')
    plot()
